...
type Tab = 'body' | 's1' | 's2' | 'calc';
const TABS: { id: Tab; label: string }[] = [
  { id: 'body', label: 'Корпус' },
  { id: 's1', label: '1 ступень' },
  { id: 's2', label: '2 ступень' },
...
export default function BuilderPanel({ design, stats, mobile, onChange, onLaunch, onPreset }: Props) {
  ...
  const checkList = (
    <div className="checks">
      {checks.map((c, i) => (
        <div key={i} className={`check check-${c.level}`}><span className="check-dot" />{c.text}</div>
      ))}
    </div>
  );
  const presets = (
      ...
      <div className="sheet">
        ...
        <div className="sheet-body">
          {tab === 'body' && (<>
            <p className="brand-sub">
              Соберите ракету, проверьте расчёт и запускайте. Плохая компоновка развалится
              в атмосфере — хорошая выйдет на орбиту.
            </p>
            {noseGroup}{finGroup}
          </>)}
        ...
        <div className="sheet-foot">
          <div className="mini-stats">
            <Mini label="ТВР" value={fmt(stats.twr, 2)} tone={stats.twr < 1.05 ? 'bad' : stats.twr > 2.6 ? 'warn' : 'ok'} />
            <Mini label="Δv" value={fmt(stats.dvTotal)} tone={stats.dvTotal < 3400 ? 'bad' : stats.dvTotal < 3900 ? 'warn' : 'ok'} />
            <Mini label="Уст." value={fmt(stats.stabilityFull, 1)}
              tone={stats.stabilityFull < 0.15 ? 'bad' : stats.stabilityFull < 0.7 ? 'warn' : 'ok'} />
            <Mini label="Масса" value={`${fmt(stats.wetMass / 1000, 1)} т`} />
          </div>
          {launchBtn}
        </div>
      </div>
  ...
  return (
    <>
      <aside className="panel panel-left">
        ...
        {noseGroup}{finGroup}{payloadGroup}
      </aside>

      <aside className="panel panel-right">
        {s1EngineGroup}{s1TankGroup}{s2Group}
      </aside>
...
function Mini({ label, value, tone }: { label: string; value: string; tone?: 'ok' | 'warn' | 'bad' }) {
  return (
    <div className="mini">
      <span className="mini-label">{label}</span>
      <span className={`mini-value${tone ? ` stat-${tone}` : ''}`}>{value}</span>
    </div>
  );
}
